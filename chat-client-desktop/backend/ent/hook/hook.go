...
// The ChatmateFunc type is an adapter to allow the use of ordinary
// function as Chatmate mutator.
type ChatmateFunc func(context.Context, *ent.ChatmateMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ChatmateFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ChatmateMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ChatmateMutation", m)
	}
	return f(ctx, mv)
}
...
func (f GroupMessageFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.GroupMessageMutation)
	if !ok {
